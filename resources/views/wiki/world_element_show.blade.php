...
@php
    ...
    // Add newlines before **Chapter just in case the AI messed up formatting
    $notes = preg_replace('/(?<!\n)(\*\*Chapter \d+:?\*\*)/i', "\n$1", $notes);
    $lines = explode("\n", $notes);
...
<div style="margin-bottom:32px;">
    <a href="{{ route('books.wiki.world-elements', $book) }}" class="nwp-text-sm nwp-text-muted" style="display:inline-flex; align-items:center; gap:4px; text-decoration:none; margin-bottom:16px;">
        &larr; Back to World Elements
    </a>
</div>

<!-- Header Card -->
<div class="nwp-card" style="padding:0; overflow:hidden; margin-bottom:40px; position:relative;">
    <div style="height:120px; background:linear-gradient(135deg, #8b5cf640, var(--color-bg-secondary)); position:relative;"></div>
    <div style="padding:0 32px 32px; display:flex; flex-direction:column; align-items:center; margin-top:-60px;">
        <div style="margin-bottom:20px; position:relative; z-index:10;">
            @if($worldElement->image_path)
                <img src="{{ Storage::url($worldElement->image_path) }}" alt="{{ $worldElement->name }}" style="width:120px; height:120px; border-radius:50%; border:4px solid var(--color-bg); box-shadow:0 8px 24px rgba(0,0,0,0.2); object-fit:cover; background:var(--color-bg-secondary);">
            @else
        ...
        <h1 class="nwp-heading" style="margin:0 0 12px 0; font-size:36px; text-align:center;">{{ $worldElement->name }}</h1>
        <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap; justify-content:center;">
            <span style="padding:6px 16px; border-radius:99px; font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); color:#8b5cf6;">
                WORLD ELEMENT
            </span>
...
<div style="display:grid; grid-template-columns:minmax(0, 1fr) minmax(0, 1fr); gap:24px; margin-bottom:40px;">
    <div class="nwp-card" style="padding:24px; grid-column:span 2;">
        <h3 class="nwp-heading" style="font-size:16px; text-transform:uppercase; letter-spacing:0.05em; color:var(--color-text-muted); margin-bottom:16px; border-bottom:1px solid var(--color-border); padding-bottom:12px;">Description</h3>
        <div style="font-size:15px; line-height:1.6; color:var(--color-text-primary);">
            {!! formatWikiTrait($worldElement->description) !!}
        </div>
    </div>
    <div class="nwp-card" style="padding:24px; grid-column:span 2;">
        <h3 class="nwp-heading" style="font-size:16px; text-transform:uppercase; letter-spacing:0.05em; color:var(--color-text-muted); margin-bottom:16px; border-bottom:1px solid var(--color-border); padding-bottom:12px;">Rules & Laws</h3>
        <div style="font-size:15px; line-height:1.6; color:var(--color-text-primary);">
            {!! formatWikiTrait($worldElement->rules_laws) !!}
        </div>
    </div>
</div>
        ...
        <div style="position:relative; padding-left:24px; border-left:2px solid var(--color-border);">
            @foreach($developments as $dev)
                <div style="position:relative; margin-bottom:32px;">
                    <!-- Timeline Dot -->
                    <div style="position:absolute; left:-33px; top:4px; width:16px; height:16px; border-radius:50%; background:var(--color-bg); border:4px solid #8b5cf6;"></div>
                    
                    <h4 class="nwp-heading" style="font-size:18px; margin:0 0 8px 0; color:#8b5cf6;">{{ $dev['title'] }}</h4>
